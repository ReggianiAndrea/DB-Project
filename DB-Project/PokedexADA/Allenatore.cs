namespace PokedexADA
{
    internal class Allenatore : Persona
    {
        ...
        //ANDRE:pensavo di rimuovere la gestione incontri da qua e usare un Pokemon Manager in PokemonInteractions
        ...
        {
            var manager = pokemonManager; // alias locale

            string output = $"Allenatore: {nickNameAllenatore} (ID: {idAllenatore})\n";

            output += $"Pokémon incontrati ({manager.PokemonIncontrati.Count}):\n";
            if (manager.PokemonIncontrati.Count == 0)
            {
                output += "— Nessun Pokémon incontrato.\n";
            }
            else
            {
                foreach (var incontrato in manager.PokemonIncontrati)
                {
                    output += $"- {incontrato}\n";
                }
            }

            output += $"Pokémon catturati ({manager.PokemonCatturati.Count}):\n";
            if (manager.PokemonCatturati.Count == 0)
            {
                output += "Nessun Pokémon catturato.\n";
            }
            else
            {
                foreach (var incontrato in manager.PokemonCatturati)
                {
                    output += $"- {incontrato}-\n";
                }
            }

            output += $"Incontri globali: {manager.NumeroIncontri}\n";
            output += $"Catture globali: {manager.NumeroCatture}\n";
